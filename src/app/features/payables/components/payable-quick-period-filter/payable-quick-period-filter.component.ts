...
@Component({
  ...
  styleUrls: ['./payable-quick-period-filter.component.scss'],
})
export class PayableQuickPeriodFilterComponent implements OnChanges {
  ...
  private singleDay(date: Date): PayableQuickPeriodRange {
    const formattedDate = this.formatDate(date);
    return { startDate: formattedDate, endDate: formattedDate };
  }

  private weekRange(date: Date): PayableQuickPeriodRange {
  ...
  private monthRange(year: number, month: number): PayableQuickPeriodRange {
