...
export class PositionService {
  ...
  insert(dto: PositionInsertDTO): Observable<PositionDTO> {
    return this.http.post<PositionDTO>(API.POSITIONS.ROOT, dto);
  }

  findById(id: number | string): Observable<PositionDetailsDTO> {
    return this.http.get<PositionDetailsDTO>(API.POSITIONS.BY_ID(id));
  }

  update(id: number, dto: PositionUpdateDTO): Observable<PositionDTO> {
    return this.http.put<PositionDTO>(API.POSITIONS.BY_ID(id), dto);
  }
